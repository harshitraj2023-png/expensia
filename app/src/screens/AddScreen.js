...
import { colors, spacing, money, MAX_AMOUNT } from '../theme';
...
export default function AddScreen() {
  ...
  const load = useCallback(async (pullToRefresh) => {
    ...
    try {
      ...
      setCategoryId((current) =>
        current && !list.some((category) => category.id === current) ? null : current
      );
      setLoadError('');
    } catch (e) {
      if (seq !== loadSeq.current) return;
  ...
  useFocusEffect(
    useCallback(() => {
      if (!dateTouched) setDate(toDateString(new Date()));
      load(false);
    }, [dateTouched, load])
  );
  ...
  async function save() {
    ...
    if (value > MAX_AMOUNT) {
      setMessage('');
      setError(`That is too large to store. The most this app takes is ${money(MAX_AMOUNT)}`);
      return;
    }
      ...
      {type === 'income' ? (
        <View>
          ...
          <TextInput
            ...
            returnKeyType="next"
            onSubmitEditing={() => noteRef.current && noteRef.current.focus()}
          />
        </View>
      ) : null}
