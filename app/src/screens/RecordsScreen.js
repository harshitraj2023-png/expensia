...
import { colors, spacing, money, sanitizeAmount, MAX_AMOUNT } from '../theme';
...
export default function RecordsScreen() {
  ...
  async function saveDraft() {
    ...
    if (amount > MAX_AMOUNT) {
      Alert.alert('Amount is too large', `The most this app takes is ${money(MAX_AMOUNT)}.`);
      return;
    }
    const body = { amount, date: draft.date, note: draft.note };
              ...
              {draft ? (
                <>
                  ...
                  <TextInput
                    style={styles.input}
                    value={draft.amount}
                    onChangeText={(text) => setDraft({ ...draft, amount: sanitizeAmount(text) })}
