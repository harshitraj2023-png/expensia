...
export default function ReportsScreen() {
  ...
  const load = useCallback(
    async (pullToRefresh) => {
      ...
      try {
        const data = await apiGet(`/summary/year?year=${year}`);
  ...
  useFocusEffect(
    useCallback(() => {
      if (!yearTouched) setYear(new Date().getFullYear());
      load(false);
    }, [yearTouched, load])
  );
  ...
  return (
    ...
    >
      <View style={styles.yearBar}>
        <Pressable style={styles.arrow} onPress={() => changeYear(year - 1)}>
          ...
          ranked.map((category, index) => {
            const share =
              report.expense_total > 0
                ...
                : 0;
            return (
              <View key={`${index}-${category.name}`} style={styles.categoryRow}>
